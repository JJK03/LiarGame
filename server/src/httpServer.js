...
io.on('connection', (socket) => {
    ...
    socket.on('requestSecondRound', ({ roomId }) => {
        ...
        if (room && room.host === socket.id) {
            ...
            room.round++; // 라운드 1 증가
            room.currentPlayerIndex = 0;
            room.turnOrder = [...room.players].sort(() => Math.random() - 0.5).map(p => p.id); // 순서 다시 섞기
        ...
        if (room && voter && !room.votes[voter.id]) {
            ...
            if (allVoted) {
                ...
                if (votedPlayerId === room.liar) {
                    resultMessage = `라이어를 찾아냈습니다! 승자는 시민입니다.\n라이어: ${liarName}`;
                } else {
                    resultMessage = `라이어를 찾지 못했습니다. 승자는 라이어입니다.\n라이어: ${liarName}`;
                }
                ...
                room.votes = {};
                io.emit('updateRoomList', Object.values(rooms).map(r => ({ id: r.id, name: r.name, playerCount: r.players.length, gameStarted: r.gameStarted })));
                io.to(roomId).emit('roomInfo', room); // 방에 있는 모든 사람에게 업데이트된 방 정보 전송
